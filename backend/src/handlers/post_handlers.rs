...
pub async fn list_all(State(pool): State<PgPool>) -> Result<Json<Vec<post>>, StatusCode> {
    ...
    let query = sqlx::query_as::<_, post>("SELECT id, postname, email, password, title, created_at FROM posts");
...
pub async fn get_by_id(Path(id): Path<i32>, State(pool): State<PgPool>) -> Result<Json<post>, StatusCode> {
    ...
    let query = sqlx::query_as::<_, post>("SELECT id, postname, email, password, title, created_at FROM posts WHERE id = $1").bind(id);
...
pub async fn delete_post(Path(id): Path<i32>, State(pool): State<PgPool>) -> StatusCode {
    ...
    let query = sqlx::query("DELETE FROM post WHERE id = $1;").bind(id);
